...
>(({ className, children, position = "popper", ...props }, ref) => (
  <SelectPrimitive.Portal>
    <SelectPrimitive.Content
      ref={ref}
      className={cn(
        "relative z-50 max-h-[min(20rem,50vh)] min-w-[8rem] overflow-hidden rounded-lg border border-border bg-popover text-popover-foreground shadow-md",
    ...
    >
      ...
      <SelectContent>
        ...
        {(() => {
          ...
          const selectedInSlice = !value || sliced.some((o) => o.id === value)
          const extraSelected = !selectedInSlice ? options.find((o) => o.id === value) : null
          return (
            <>
              {extraSelected && (
                <SelectItem key={extraSelected.id} value={extraSelected.id} className="hidden">
                  {extraSelected.label}
                </SelectItem>
              )}
              {sliced.map((opt) => (
